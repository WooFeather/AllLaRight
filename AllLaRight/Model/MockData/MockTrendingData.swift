...
/// #2. 트렌딩화면
struct MockTrendingCoinItem {
  let item: MockTrendingCoinDetails
}
...
struct MockTrendingCategoryItem {
  let id: Int
  let name: String
  let marketCap1hChange: Double
  let slug: String
  let coinsCount: Int
  let data: MockTrendingCategoryData
}

struct MockTrendingCategoryData {
...
let mockTrendingCoins: [MockTrendingCoinItem] = [
  // Pixels (PIXEL)
  ...
  MockTrendingCoinItem(item: MockTrendingCoinDetails(
    ...
    data: MockTrendingCoinData(
      ...
      content: MockTrendingContentInfo(
...
// 인기 NFT 목데이터
let mockTrendingNFTs: [MockTrendingNFTItem] = [
  // ARCStellars (ARS)
  MockTrendingNFTItem(
    id: "arc-stellars",
    name: "ARCStellars",
